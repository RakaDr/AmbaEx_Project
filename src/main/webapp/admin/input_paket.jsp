...
<div class="container-fluid">
    <div class="row">
        ...
        <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4 py-4">
            ...
            <form action="../PaketServlet" method="POST">
                ...
                <div class="card p-4">
                    <div class="card-body">
                        <div class="row mb-4">
                            ...
                            <div class="col-md-6"><label class="fw-bold mb-2">Tanggal Input</label><input type="text" class="form-control bg-light" value="<%= new java.util.Date() %>" readonly></div>
                        </div>
                        ...
                        <div class="row">
                            <div class="col-md-6 pe-md-4 border-end">
                                ...
                                <div class="mb-3"><label class="form-label text-muted small fw-bold">KOTA ASAL</label><select class="form-select" name="kota_asal"><option value="Jakarta">Jakarta</option><option value="Bandung">Bandung</option><option value="Surabaya">Surabaya</option><option value="Semarang">Semarang</option><option value="Medan">Medan</option><option value="Bekasi">Bekasi</option></select></div>
                            </div>
                            <div class="col-md-6 ps-md-4">
                                ...
                                <div class="mb-3"><label class="form-label text-muted small fw-bold">KOTA TUJUAN</label><select class="form-select" name="kota_tujuan"><option value="Jakarta">Jakarta</option><option value="Bandung">Bandung</option><option value="Surabaya">Surabaya</option><option value="Semarang">Semarang</option><option value="Medan">Medan</option><option value="Bekasi">Bekasi</option></select></div>
                            </div>
                        </div>
                        <hr class="my-4">
                        <div class="section-title">Detail & Biaya</div>
                        <div class="row g-3">
                            <div class="col-md-3"><label class="form-label fw-bold">Berat (Kg)</label><input type="number" id="beratInput" step="0.1" name="berat" class="form-control" oninput="hitungBiaya()" required></div>
                            <div class="col-md-3">
                                <label class="form-label fw-bold">Layanan</label>
                                <select class="form-select" id="layananInput" name="layanan" onchange="hitungBiaya()">
                                    <% LayananDAO layDao = new LayananDAO(); List<Layanan> listLay = layDao.getAllLayanan(); if (listLay != null) { for(Layanan l : listLay) { %>
                                        <option value="<%= l.getNamaLayanan() %>" data-harga="<%= l.getHargaPerKg() %>"><%= l.getNamaLayanan() %></option>
                                    <% } } %>
                                </select>
                            </div>
                            <div class="col-md-3"><label class="form-label fw-bold">Isi Paket</label><input type="text" name="isi_paket" class="form-control"></div>
                            <div class="col-md-3"><label class="form-label fw-bold text-success">Total Biaya</label><div class="input-group"><span class="input-group-text bg-success border-success text-white">Rp</span><input type="number" id="biayaInput" name="biaya" class="form-control fw-bold text-success border-success" readonly></div></div>
                        </div>
...
<script>
    ...
    function hitungBiaya() {
        ...
        let select = document.getElementById('layananInput');
        let hargaPerKg = 0;
        if(select.selectedIndex !== -1) { hargaPerKg = parseFloat(select.options[select.selectedIndex].getAttribute('data-harga')) || 0; }
        document.getElementById('biayaInput').value = berat * hargaPerKg;
    }
    document.addEventListener("DOMContentLoaded", function() { hitungBiaya(); });
